...
int main(){
	...
	// AUTH PHASE
	/*if((ret = authenticate(socket_d, &this_user)) < 0){
		error_handler("authentication to the server failed. Exit program...");
		close(socket_d);
		exit(0);
	}*/
	...
	while(1) {
		...
		if(file1){
			...
			/*if((ret = access((char*)path1, F_OK)) < 0){
				error_handler("File not found. Aborting operation...");
				free_var(1);
				continue;
			}
			else
				cout << "file found, ready to send" << endl;*/
		}
		if(file2)
